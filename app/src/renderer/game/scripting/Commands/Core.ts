...
const setFpsCommand = makeSimple((context, args) =>
  Effect.gen(function* () {
    ...
    yield* context.run(context.settings.setFrameRate(Math.max(1, Math.floor(fps))));
  }),
);
...
export const coreScriptCommands: ReadonlyArray<readonly [string, ScriptCommandHandler]> =
  [
    ...
    ["enable_hideplayers", makeSimple((context) => context.run(context.settings.setPlayersVisible(true)))],
    ["disable_hideplayers", makeSimple((context) => context.run(context.settings.setPlayersVisible(false)))],
