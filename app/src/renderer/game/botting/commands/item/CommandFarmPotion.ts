...
export class CommandFarmPotion extends Command {
    ...
    private async buyReagentFromShop(
        ...
        this.logger.debug(`Shop ${source.shopId} loaded: ${this.bot.shops.isShopLoaded()}, isMerge: ${isMergeShop}`);
        ...
        if (source.buyShopItemId) {
            ...
            this.logger.debug(`Buying via buyByShopItemId(${source.buyShopItemId}, ${quantity}), canBuy: ${canBuy}`);

            if (isMergeShop) {
                for (let idx = 0; idx < quantity; idx++) {
                    if (!this.bot.shops.canBuyItem(reagent)) {
                        this.logger.warn(`Cannot merge ${reagent} at iteration ${idx + 1}/${quantity}`);
                        break;
                    }
            ...
            const canBuy = this.bot.shops.canBuyItem(reagent);
            this.logger.debug(`Buying via buyById(${source.shopItemId}, ${quantity}), canBuy: ${canBuy}`);
     ...
     */
    private async buyMergeRequirements(itemName: string, quantity: number): Promise<void> {
        ...
        for (const req of shopItem.data.turnin) {
            ...
            if (needed > 0) {
                this.logger.debug(`Need ${needed}x ${req.sName} (have ${currentQty}, need ${totalNeeded})`);
                ...
                if (this.bot.shops.canBuyItem(req.sName)) {
                    ...
                    this.logger.warn(`Cannot buy merge requirement: ${req.sName} - check if you have enough gold!`);
                }
            }
        }
    }
    ...
    private async buyDragonRunestone(quantity: number): Promise<void> {
        ...
        this.logger.debug(`Shop loaded: ${this.bot.shops.isShopLoaded()}, isMerge: ${this.bot.shops.isMergeShop}`);

        const currentRunestones = this.bot.inventory.get("Dragon Runestone")?.quantity ?? 0;
        const vouchersNeeded = quantity - currentRunestones;

        if (vouchersNeeded <= 0) return;

        const currentVouchers = this.bot.inventory.get("Gold Voucher 100k")?.quantity ?? 0;
        ...
        if (vouchersToBuy > 0) {
            ...
            this.logger.debug(`Buying ${vouchersToBuy}x Gold Voucher 100k... canBuy: ${canBuyVoucher}`);

            if (canBuyVoucher) {
                await this.bot.shops.buyByName("Gold Voucher 100k", vouchersToBuy);
                await this.bot.sleep(500);
                this.logger.debug(`After buy: ${this.bot.inventory.get("Gold Voucher 100k")?.quantity ?? 0}x Gold Voucher 100k`);
            } else {
        ...
        for (let idx = 0; idx < vouchersNeeded; idx++) {
            if (!this.bot.shops.canBuyItem("Dragon Runestone")) {
                this.logger.warn(`Cannot merge Dragon Runestone at iteration ${idx + 1}`);
                break;
            }
        ...
        this.logger.debug(`After merge: ${this.bot.inventory.get("Dragon Runestone")?.quantity ?? 0}x Dragon Runestone`);
    }
    ...
    private async performAlchemy(
        ...
        if (!reagent1Item || !reagent2Item) {
            this.logger.warn(`Missing reagent: ${reagent1Item ? recipe.reagent2 : recipe.reagent1}`);
            return;
        }
        ...
        this.logger.info(`Starting alchemy: ${recipe.reagent1} + ${recipe.reagent2} -> ${recipe.name}`);
        ...
        const hasPotionTarget = this.bot.inventory.contains(recipe.name, targetQuantity);

        this.logger.debug(`Reagent1 (${recipe.reagent1}): ${hasReagent1}, qty: ${this.bot.inventory.get(recipe.reagent1)?.quantity ?? 0}`);
        this.logger.debug(`Reagent2 (${recipe.reagent2}): ${hasReagent2}, qty: ${this.bot.inventory.get(recipe.reagent2)?.quantity ?? 0}`);
        this.logger.debug(`Dragon Runestone: ${hasRunestone}, qty: ${this.bot.inventory.get("Dragon Runestone")?.quantity ?? 0}`);
        this.logger.debug(`Already have ${recipe.name}: ${hasPotionTarget}, qty: ${this.bot.inventory.get(recipe.name)?.quantity ?? 0}/${targetQuantity}`);
        ...
        while (
            ...
            this.logger.info(`Created potion - now have ${currentQty}/${targetQuantity} ${recipe.name}`);
        }
    }
