...
export class CommandEnhanceItem extends Command {
  ...
  private resolveStrategy(ctx: ItemContext): EnhancementStrategy | null {
    ...
    this.logger.warn(
      `Unknown enhancement: ${this.enhancementName}${this.procName ? ` (${this.procName})` : ""}`,
    );
    return null;
  }
  ...
  private resolveBasicStrategy(
    ctx: ItemContext,
    basicType: EnhancementType,
    procInput: string,
  ): EnhancementStrategy | null {
    if (!ctx.isWeapon && !ctx.isCape && !ctx.isHelm && !ctx.isArmor) {
      this.logger.warn(
        `Cannot enhance item "${this.itemName}" - unsupported item type`,
      );
      return null;
    }
  ...
  private findBestEnhancement(
    ...
    const candidates = shopInfo.items.filter(
      (shopItem) =>
  ...
  private canPurchase(
    ...
    if (requiredRep > 0 && factionId > 0) {
      const faction = this.bot.player.factions
        .all()
        .find((fac) => fac.id === factionId);
    ...
    if (questSlot >= 0) {
      const currentValue = this.bot.flash.call<number>(
        "world.getQuestValue",
        questSlot,
      );
  ...
  private matchesPattern(
    ...
    const itemPatternId = Number(
      shopItem["PatternID"] ?? shopItem["EnhPatternID"] ?? 0,
    );
    const itemProcId = Number(
      shopItem["ItemProcID"] ?? shopItem["ProcID"] ?? 0,
    );
