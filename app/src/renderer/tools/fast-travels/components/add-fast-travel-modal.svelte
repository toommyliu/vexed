...
<Dialog.Root open={isOpen} onOpenChange={(open) => !open && onClose()}>
  <Dialog.Content showCloseButton={true} class="sm:max-w-md">
    ...
    <Dialog.Header class="pb-2">
      <Dialog.Title class="text-lg font-semibold tracking-tight">Add Location</Dialog.Title>
    </Dialog.Header>

    <form id="add-fast-travel-form" onsubmit={handleSubmit} class="grid gap-4 px-6">
      {#if error}
        {#key error}
          <div
      ...
      <div class="grid grid-cols-2 gap-3">
        <div class="grid gap-2">
          <Label for="add-cell" class="text-sm font-medium text-muted-foreground">Cell <span class="text-xs">(optional)</span></Label>
          <Input
            id="add-cell"
            bind:value={cell}
            disabled={isSubmitting}
        ...
        <div class="grid gap-2">
          <Label for="add-pad" class="text-sm font-medium text-muted-foreground">Pad <span class="text-xs">(optional)</span></Label>
          <Input
            id="add-pad"
            bind:value={pad}
            disabled={isSubmitting}
