...
const renderTypesIndex = (types: readonly ReferencedTypeDoc[]): string => {
  const lines = [
    ...
    ...types.map(
      (type) =>
        `| [\`${type.name}\`](./${type.slug}/) | ${escapeTableCell(
          type.kind,
        )} | ${escapeTableCell(type.summary)} |`,
    ),
  ];
  return finalizeMarkdown(lines);
};
...
const renderFiles = (
  ...
  const rootTypeLinks = buildTypeLinks(types, "./types/");
  const apiTypeLinks = buildTypeLinks(types, "./types/");
  const nestedApiTypeLinks = buildTypeLinks(types, "../../types/");
  return [
    {
    ...
    },
    {
      ...
      content: renderApiOverview(helpers, namespaces, apiTypeLinks),
    },
    {
    ...
    ...groups.map((group) => ({
      ...
      content: renderGroup(
        group,
        group.id.startsWith("api/") ? nestedApiTypeLinks : rootTypeLinks,
      ),
    })),
    ...types.map((type) => ({
