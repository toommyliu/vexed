---
title: Conditions Commands
description: Generated vexed conditions command reference.
sidebar:
  label: Conditions
tableOfContents:
  minHeadingLevel: 2
  maxHeadingLevel: 2
---

<!-- Generated by pnpm run docgen. Do not edit directly. -->

## Reference Index

### Commands

| Command | Summary |
| --- | --- |
| [`cmd.and`](#command-and) | Builds a logical AND condition expression.<br><br>`and` only creates an expression. It does not start a block by itself;<br>pass the result to `if`, `not`, `or`, or another `and`. Use `if_all`<br>when the entire conditional block is a flat AND of several expressions. |
| [`cmd.any_player_hp_percentage`](#command-any_player_hp_percentage) | Builds an any-player HP percentage condition expression. |
| [`cmd.aura_stacks`](#command-aura_stacks) | Builds a local-player aura stack-count condition expression.<br><br>Stack count means repeated active aura instances with the same aura name.<br>Use [`cmd.aura_value`](#command-aura_value) to compare the optional numeric `val` in the aura payload. |
| [`cmd.aura_value`](#command-aura_value) | Builds a local-player aura value condition expression.<br><br>Aura value refers to the optional numeric `val` in the aura payload, which can be an<br>integer or float. Use [`cmd.aura_stacks`](#command-aura_stacks) to compare repeated aura instances. |
| [`cmd.can_buy_item`](#command-can_buy_item) | Checks whether a quantity of an item can be bought from the loaded shop. |
| [`cmd.can_complete_quest`](#command-can_complete_quest) | Checks whether a quest can be completed. |
| [`cmd.cannot_complete_quest`](#command-cannot_complete_quest) | Checks whether a quest cannot be completed. |
| [`cmd.cell_player_count`](#command-cell_player_count) | Builds a cell player count condition expression. |
| [`cmd.class_rank`](#command-class_rank) | Builds a class rank condition expression. |
| [`cmd.else`](#command-else) | Switches to the false branch of an active conditional block. |
| [`cmd.end_if`](#command-end_if) | Ends the current conditional block. |
| [`cmd.equipped`](#command-equipped) | Checks whether an item is equipped. |
| [`cmd.faction_rank`](#command-faction_rank) | Builds a faction rank condition expression. |
| [`cmd.gold`](#command-gold) | Builds a gold condition expression. |
| [`cmd.has_aura`](#command-has_aura) | Builds a local-player aura presence condition expression. |
| [`cmd.has_no_target`](#command-has_no_target) | Checks whether the player currently has no target. |
| [`cmd.has_player_aura`](#command-has_player_aura) | Builds a player aura presence condition expression. |
| [`cmd.has_target`](#command-has_target) | Checks whether the player currently has a target. |
| [`cmd.hp`](#command-hp) | Builds a self HP condition expression. |
| [`cmd.hp_percentage`](#command-hp_percentage) | Builds a self HP percentage condition expression. |
| [`cmd.if`](#command-if) | Starts a conditional block from one complete condition expression.<br><br>`if` controls whether the following commands run. It accepts a single<br>expression, which can be a simple condition or a nested expression built<br>with `and`, `or`, or `not`. |
| [`cmd.if_all`](#command-if_all) | Starts a conditional block that runs only when every expression matches.<br><br>`if_all(a, b)` is control-flow shorthand for<br>`if(and(a, b))`. Use it when the whole block has a flat AND condition.<br>Use `and` inside `if` when you need to nest the AND with `or` or `not`. |
| [`cmd.if_any`](#command-if_any) | Starts a conditional block that runs when any expression matches.<br><br>`if_any(a, b)` is control-flow shorthand for<br>`if(or(a, b))`. Use it when the whole block has a flat OR condition.<br>Use `or` inside `if` when you need to nest the OR with `and` or `not`. |
| [`cmd.in_bank`](#command-in_bank) | Checks whether the bank contains an item. |
| [`cmd.in_cell`](#command-in_cell) | Checks whether the player is in a cell. |
| [`cmd.in_combat`](#command-in_combat) | Checks whether the player is in combat. |
| [`cmd.in_house`](#command-in_house) | Checks whether the house inventory contains an item. |
| [`cmd.in_inventory`](#command-in_inventory) | Checks whether the inventory contains an item. |
| [`cmd.in_map`](#command-in_map) | Checks whether the player is in a map. |
| [`cmd.in_tempinventory`](#command-in_tempinventory) | Checks whether the temp inventory contains an item. |
| [`cmd.is_maxed`](#command-is_maxed) | Checks whether an item is at its max stack quantity.<br>For class items, maxed means the class has reached rank 10. |
| [`cmd.is_member`](#command-is_member) | Checks whether the account is a member. |
| [`cmd.is_not_maxed`](#command-is_not_maxed) | Checks whether an item is not at its max stack quantity. |
| [`cmd.is_not_member`](#command-is_not_member) | Checks whether the account is not a member. |
| [`cmd.item_has_dropped`](#command-item_has_dropped) | Checks whether an item is present in the drop list. |
| [`cmd.item_has_not_dropped`](#command-item_has_not_dropped) | Checks whether an item is absent from the drop list. |
| [`cmd.level`](#command-level) | Builds a level condition expression. |
| [`cmd.monster_hp`](#command-monster_hp) | Builds a monster HP condition expression. |
| [`cmd.monster_hp_percentage`](#command-monster_hp_percentage) | Builds a monster HP percentage condition expression. |
| [`cmd.monster_in_room`](#command-monster_in_room) | Checks whether a monster is present in the room. |
| [`cmd.monster_not_in_room`](#command-monster_not_in_room) | Checks whether a monster is absent from the room. |
| [`cmd.mp`](#command-mp) | Builds a self MP condition expression. |
| [`cmd.mp_percentage`](#command-mp_percentage) | Builds a self MP percentage condition expression. |
| [`cmd.not`](#command-not) | Builds the inverse of a condition expression.<br><br>`not` only creates an expression. It does not start a block by itself;<br>pass it to `if`, `if_all`, `if_any`, `and`, or `or`. |
| [`cmd.not_equipped`](#command-not_equipped) | Checks whether an item is not equipped. |
| [`cmd.not_in_bank`](#command-not_in_bank) | Checks whether the bank does not contain an item. |
| [`cmd.not_in_cell`](#command-not_in_cell) | Checks whether the player is not in a cell. |
| [`cmd.not_in_combat`](#command-not_in_combat) | Checks whether the player is not in combat. |
| [`cmd.not_in_house`](#command-not_in_house) | Checks whether the house inventory does not contain an item. |
| [`cmd.not_in_inventory`](#command-not_in_inventory) | Checks whether the inventory does not contain an item. |
| [`cmd.not_in_map`](#command-not_in_map) | Checks whether the player is not in a map. |
| [`cmd.not_in_tempinventory`](#command-not_in_tempinventory) | Checks whether the temp inventory does not contain an item. |
| [`cmd.or`](#command-or) | Builds a logical OR condition expression.<br><br>`or` only creates an expression. It does not start a block by itself;<br>pass the result to `if`, `not`, `and`, or another `or`. Use `if_any`<br>when the entire conditional block is a flat OR of several expressions. |
| [`cmd.player_aura_stacks`](#command-player_aura_stacks) | Builds a player aura stack-count condition expression.<br><br>Stack count means repeated active aura instances with the same aura name.<br>Use [`cmd.player_aura_value`](#command-player_aura_value) to compare the optional numeric `val` in the aura payload. |
| [`cmd.player_aura_value`](#command-player_aura_value) | Builds a player aura value condition expression.<br><br>Aura value refers to the optional numeric `val` in the aura payload, which can be an<br>integer or float. Use [`cmd.player_aura_stacks`](#command-player_aura_stacks) to compare repeated aura<br>instances. |
| [`cmd.player_count`](#command-player_count) | Builds a room player count condition expression. |
| [`cmd.player_hp`](#command-player_hp) | Builds a named player HP condition expression. |
| [`cmd.player_hp_percentage`](#command-player_hp_percentage) | Builds a named player HP percentage condition expression. |
| [`cmd.player_in_cell`](#command-player_in_cell) | Checks whether a player is in a cell. |
| [`cmd.player_in_map`](#command-player_in_map) | Checks whether a player is in the current map. |
| [`cmd.player_name_equals`](#command-player_name_equals) | Checks whether the current player name matches a value. |
| [`cmd.player_not_in_cell`](#command-player_not_in_cell) | Checks whether a player is not in a cell. |
| [`cmd.player_not_in_map`](#command-player_not_in_map) | Checks whether a player is not in the current map. |
| [`cmd.quest_in_progress`](#command-quest_in_progress) | Checks whether a quest is in progress. |
| [`cmd.quest_is_available`](#command-quest_is_available) | Checks whether a quest is available. |
| [`cmd.quest_not_available`](#command-quest_not_available) | Checks whether a quest is not available. |
| [`cmd.quest_not_in_progress`](#command-quest_not_in_progress) | Checks whether a quest is not in progress. |
| [`cmd.target_hp`](#command-target_hp) | Builds a current target HP condition expression. |

### Referenced Types

| Type | Kind | Summary |
| --- | --- | --- |
| [`ConditionInput`](#type-conditioninput) | type |  |
| [`ScriptAuraMetric`](#type-scriptaurametric) | type |  |
| [`ScriptComparisonOperator`](#type-scriptcomparisonoperator) | type |  |
| [`ScriptCondition`](#type-scriptcondition) | type | Serializable condition expression produced by the scripting DSL.<br><br>Condition builder methods in `conditions.ts` create these plain objects<br>during script compilation. Control-flow commands such as [`cmd.if(...)`](#command-if)<br>record the object in a `ScriptInstruction`, and `evaluateScriptCondition`<br>interprets it at runtime by dispatching on `_tag`.<br><br>Example flow:<br>`cmd.if(cmd.hp("<", 1000))`<br>  1. [`cmd.hp`](#command-hp) builds `{ _tag: "PlayerMetric", metric: "hp", ... }`.<br>  2. [`cmd.if`](#command-if) stores that object as the first instruction argument.<br>  3. `ifCommand` calls `evaluateScriptCondition` with that argument.<br>  4. The `"PlayerMetric"` case reads live HP and compares it to `1000`. |
| [`ScriptInventoryLocation`](#type-scriptinventorylocation) | type |  |
| [`ScriptMonsterMetric`](#type-scriptmonstermetric) | type |  |
| [`ScriptPlayerMetric`](#type-scriptplayermetric) | type |  |

## Commands

<a id="command-and"></a>

### `cmd.and(...conditions)`

Builds a logical AND condition expression.

`and` only creates an expression. It does not start a block by itself;
pass the result to `if`, `not`, `or`, or another `and`. Use `if_all`
when the entire conditional block is a flat AND of several expressions.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1323`

```ts
cmd.and(...conditions: ConditionInput[]): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `conditions` | [`ConditionInput`](../conditions/#type-conditioninput)[] | yes |  | Complete condition expressions to combine with logical AND. |

**Examples**

```js
cmd.if(cmd.and(cmd.in_map("battleon"), cmd.hp_percentage(">", 80)))
```

<a id="command-any_player_hp_percentage"></a>

### `cmd.any_player_hp_percentage(operator, value)`

Builds an any-player HP percentage condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1079`

```ts
cmd.any_player_hp_percentage(operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | HP percentage to compare against. |

**Examples**

```js
cmd.if(cmd.any_player_hp_percentage("<", 25))
```

<a id="command-aura_stacks"></a>

### `cmd.aura_stacks(aura, operator, count)`

Builds a local-player aura stack-count condition expression.

Stack count means repeated active aura instances with the same aura name.
Use [`cmd.aura_value`](#command-aura_value) to compare the optional numeric `val` in the aura payload.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1178`

```ts
cmd.aura_stacks(aura: string, operator: ScriptComparisonOperatorInput, count: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `aura` | `string` | yes |  | Aura name. |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `count` | `number` | yes |  | Aura stack count to compare against. |

**Examples**

```js
cmd.if(cmd.aura_stacks("Some Aura", ">=", 3))
```

<a id="command-aura_value"></a>

### `cmd.aura_value(aura, operator, value)`

Builds a local-player aura value condition expression.

Aura value refers to the optional numeric `val` in the aura payload, which can be an
integer or float. Use [`cmd.aura_stacks`](#command-aura_stacks) to compare repeated aura instances.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1155`

```ts
cmd.aura_value(aura: string, operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `aura` | `string` | yes |  | Aura name. |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | Aura value to compare against. Can be an integer or float. |

**Examples**

```js
cmd.if(cmd.aura_value("Some Aura", ">=", 1))
```

<a id="command-can_buy_item"></a>

### `cmd.can_buy_item(item, quantity)`

Checks whether a quantity of an item can be bought from the loaded shop.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1878`

```ts
cmd.can_buy_item(item: string, quantity: number | undefined): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `item` | `string` | yes |  | Item name. |
| `quantity` | `number` \| `undefined` | yes |  | Quantity to check. Defaults to 1. |

<a id="command-can_complete_quest"></a>

### `cmd.can_complete_quest(questId)`

Checks whether a quest can be completed.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1652`

```ts
cmd.can_complete_quest(questId: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `questId` | `number` | yes |  | Quest id. |

<a id="command-cannot_complete_quest"></a>

### `cmd.cannot_complete_quest(questId)`

Checks whether a quest cannot be completed.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1668`

```ts
cmd.cannot_complete_quest(questId: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `questId` | `number` | yes |  | Quest id. |

<a id="command-cell_player_count"></a>

### `cmd.cell_player_count(operator, value, cell)`

Builds a cell player count condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1114`

```ts
cmd.cell_player_count(operator: ScriptComparisonOperatorInput, value: number, cell: string | undefined): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | Player count to compare against. |
| `cell` | `string` \| `undefined` | yes |  | Optional cell name. Defaults to the current cell. |

**Examples**

```js
cmd.if(cmd.cell_player_count(">=", 2, "Enter"))
```

<a id="command-class_rank"></a>

### `cmd.class_rank(className, operator, value)`

Builds a class rank condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1024`

```ts
cmd.class_rank(className: string, operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `className` | `string` | yes |  | Class item name. |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | Rank value to compare against. |

**Examples**

```js
cmd.if(cmd.class_rank("ArchPaladin", ">=", 10))
```

<a id="command-else"></a>

### `cmd.else()`

Switches to the false branch of an active conditional block.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:900`

```ts
cmd.else(): void
```

**Examples**

```js
cmd.if(cmd.in_combat())
cmd.log("fighting")
cmd.else()
cmd.log("idle")
cmd.end_if()
```

<a id="command-end_if"></a>

### `cmd.end_if()`

Ends the current conditional block.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:907`

```ts
cmd.end_if(): void
```

<a id="command-equipped"></a>

### `cmd.equipped(item)`

Checks whether an item is equipped.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1377`

```ts
cmd.equipped(item: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `item` | `string` | yes |  | Item name. |

<a id="command-faction_rank"></a>

### `cmd.faction_rank(faction, operator, value)`

Builds a faction rank condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1006`

```ts
cmd.faction_rank(faction: string, operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `faction` | `string` | yes |  | Faction name. |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | Rank value to compare against. |

**Examples**

```js
cmd.if(cmd.faction_rank("Good", ">=", 10))
```

<a id="command-gold"></a>

### `cmd.gold(operator, value)`

Builds a gold condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:981`

```ts
cmd.gold(operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | Gold value to compare against. |

**Examples**

```js
cmd.if(cmd.gold(">=", 1000000))
```

<a id="command-has_aura"></a>

### `cmd.has_aura(aura)`

Builds a local-player aura presence condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1136`

```ts
cmd.has_aura(aura: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `aura` | `string` | yes |  | Aura name. |

**Examples**

```js
cmd.if(cmd.has_aura("Some Aura"))
```

<a id="command-has_no_target"></a>

### `cmd.has_no_target()`

Checks whether the player currently has no target.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1417`

```ts
cmd.has_no_target(): ScriptCondition
```

<a id="command-has_player_aura"></a>

### `cmd.has_player_aura(player, aura)`

Builds a player aura presence condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1197`

```ts
cmd.has_player_aura(player: string, aura: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `player` | `string` | yes |  | Player name. |
| `aura` | `string` | yes |  | Aura name. |

**Examples**

```js
cmd.if(cmd.has_player_aura("Artix", "Some Aura"))
```

<a id="command-has_target"></a>

### `cmd.has_target()`

Checks whether the player currently has a target.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1407`

```ts
cmd.has_target(): ScriptCondition
```

<a id="command-hp"></a>

### `cmd.hp(operator, value)`

Builds a self HP condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:933`

```ts
cmd.hp(operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | HP value to compare against. |

**Examples**

```js
cmd.if(cmd.hp("<", 1000))
```

<a id="command-hp_percentage"></a>

### `cmd.hp_percentage(operator, value)`

Builds a self HP percentage condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:957`

```ts
cmd.hp_percentage(operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | HP percentage to compare against. |

**Examples**

```js
cmd.if(cmd.hp_percentage("<", 40))
```

<a id="command-if"></a>

### `cmd.if(condition)`

Starts a conditional block from one complete condition expression.

`if` controls whether the following commands run. It accepts a single
expression, which can be a simple condition or a nested expression built
with `and`, `or`, or `not`.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:842`

```ts
cmd.if(condition: ConditionInput): void
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `condition` | [`ConditionInput`](../conditions/#type-conditioninput) | yes |  | Complete condition expression to evaluate. |

**Examples**

```js
cmd.if(cmd.not(cmd.in_combat()))
```

<a id="command-if_all"></a>

### `cmd.if_all(...conditions)`

Starts a conditional block that runs only when every expression matches.

`if_all(a, b)` is control-flow shorthand for
`if(and(a, b))`. Use it when the whole block has a flat AND condition.
Use `and` inside `if` when you need to nest the AND with `or` or `not`.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:857`

```ts
cmd.if_all(...conditions: ConditionInput[]): void
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `conditions` | [`ConditionInput`](../conditions/#type-conditioninput)[] | yes |  | Complete condition expressions to combine with logical AND. |

**Examples**

```js
cmd.if_all(cmd.in_map("battleon"), cmd.hp_percentage(">", 80))
```

<a id="command-if_any"></a>

### `cmd.if_any(...conditions)`

Starts a conditional block that runs when any expression matches.

`if_any(a, b)` is control-flow shorthand for
`if(or(a, b))`. Use it when the whole block has a flat OR condition.
Use `or` inside `if` when you need to nest the OR with `and` or `not`.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:879`

```ts
cmd.if_any(...conditions: ConditionInput[]): void
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `conditions` | [`ConditionInput`](../conditions/#type-conditioninput)[] | yes |  | Complete condition expressions to combine with logical OR. |

**Examples**

```js
cmd.if_any(cmd.in_inventory("Token"), cmd.in_bank("Token"))
```

<a id="command-in_bank"></a>

### `cmd.in_bank(item, quantity)`

Checks whether the bank contains an item.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1494`

```ts
cmd.in_bank(item: ScriptItemIdentifier, quantity: number | undefined): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `item` | [`ScriptItemIdentifier`](../item/#type-scriptitemidentifier) | yes |  | Item name or item id. |
| `quantity` | `number` \| `undefined` | yes |  | Quantity required. Defaults to `1`. |

<a id="command-in_cell"></a>

### `cmd.in_cell(cell)`

Checks whether the player is in a cell.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1347`

```ts
cmd.in_cell(cell: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `cell` | `string` | yes |  | Cell name. |

<a id="command-in_combat"></a>

### `cmd.in_combat()`

Checks whether the player is in combat.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1511`

```ts
cmd.in_combat(): ScriptCondition
```

<a id="command-in_house"></a>

### `cmd.in_house(item, quantity)`

Checks whether the house inventory contains an item.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1534`

```ts
cmd.in_house(item: string, quantity: number | undefined): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `item` | `string` | yes |  | Item name. |
| `quantity` | `number` \| `undefined` | yes |  | Quantity required. Defaults to `1`. |

<a id="command-in_inventory"></a>

### `cmd.in_inventory(item, quantity)`

Checks whether the inventory contains an item.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1430`

```ts
cmd.in_inventory(item: string, quantity: number | undefined): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `item` | `string` | yes |  | Item name. |
| `quantity` | `number` \| `undefined` | yes |  | Quantity required. Defaults to `1`. |

<a id="command-in_map"></a>

### `cmd.in_map(map)`

Checks whether the player is in a map.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1813`

```ts
cmd.in_map(map: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `map` | `string` | yes |  | Map name. |

<a id="command-in_tempinventory"></a>

### `cmd.in_tempinventory(item, quantity)`

Checks whether the temp inventory contains an item.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1462`

```ts
cmd.in_tempinventory(item: string, quantity: number | undefined): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `item` | `string` | yes |  | Temporary item name. |
| `quantity` | `number` \| `undefined` | yes |  | Quantity required. Defaults to `1`. |

<a id="command-is_maxed"></a>

### `cmd.is_maxed(item)`

Checks whether an item is at its max stack quantity.
For class items, maxed means the class has reached rank 10.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1749`

```ts
cmd.is_maxed(item: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `item` | `string` | yes |  | Item name. |

<a id="command-is_member"></a>

### `cmd.is_member()`

Checks whether the account is a member.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1551`

```ts
cmd.is_member(): ScriptCondition
```

<a id="command-is_not_maxed"></a>

### `cmd.is_not_maxed(item)`

Checks whether an item is not at its max stack quantity.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1765`

```ts
cmd.is_not_maxed(item: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `item` | `string` | yes |  | Item name. |

<a id="command-is_not_member"></a>

### `cmd.is_not_member()`

Checks whether the account is not a member.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1561`

```ts
cmd.is_not_member(): ScriptCondition
```

<a id="command-item_has_dropped"></a>

### `cmd.item_has_dropped(item)`

Checks whether an item is present in the drop list.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1781`

```ts
cmd.item_has_dropped(item: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `item` | `string` | yes |  | Item name. |

<a id="command-item_has_not_dropped"></a>

### `cmd.item_has_not_dropped(item)`

Checks whether an item is absent from the drop list.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1797`

```ts
cmd.item_has_not_dropped(item: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `item` | `string` | yes |  | Item name. |

<a id="command-level"></a>

### `cmd.level(operator, value)`

Builds a level condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:993`

```ts
cmd.level(operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | Level value to compare against. |

**Examples**

```js
cmd.if(cmd.level("=", 100))
```

<a id="command-monster_hp"></a>

### `cmd.monster_hp(target, operator, value)`

Builds a monster HP condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1283`

```ts
cmd.monster_hp(target: string, operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `target` | `string` | yes |  | Monster name or target token. |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | HP value to compare against. |

**Examples**

```js
cmd.if(cmd.monster_hp("Ultra Boss", "<=", 100000))
```

<a id="command-monster_hp_percentage"></a>

### `cmd.monster_hp_percentage(target, operator, value)`

Builds a monster HP percentage condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1302`

```ts
cmd.monster_hp_percentage(target: string, operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `target` | `string` | yes |  | Monster name or target token. |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | HP percentage to compare against. |

**Examples**

```js
cmd.if(cmd.monster_hp_percentage("Ultra Boss", "<=", 25))
```

<a id="command-monster_in_room"></a>

### `cmd.monster_in_room(monster)`

Checks whether a monster is present in the room.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1843`

```ts
cmd.monster_in_room(monster: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `monster` | `string` | yes |  | Monster name. |

<a id="command-monster_not_in_room"></a>

### `cmd.monster_not_in_room(monster)`

Checks whether a monster is absent from the room.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1858`

```ts
cmd.monster_not_in_room(monster: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `monster` | `string` | yes |  | Monster name. |

<a id="command-mp"></a>

### `cmd.mp(operator, value)`

Builds a self MP condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:945`

```ts
cmd.mp(operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | MP value to compare against. |

**Examples**

```js
cmd.if(cmd.mp(">=", 30))
```

<a id="command-mp_percentage"></a>

### `cmd.mp_percentage(operator, value)`

Builds a self MP percentage condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:969`

```ts
cmd.mp_percentage(operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | MP percentage to compare against. |

**Examples**

```js
cmd.if(cmd.mp_percentage(">=", 20))
```

<a id="command-not"></a>

### `cmd.not(condition)`

Builds the inverse of a condition expression.

`not` only creates an expression. It does not start a block by itself;
pass it to `if`, `if_all`, `if_any`, `and`, or `or`.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:921`

```ts
cmd.not(condition: ConditionInput): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `condition` | [`ConditionInput`](../conditions/#type-conditioninput) | yes |  | Complete condition expression to invert. |

**Examples**

```js
cmd.if(cmd.not(cmd.in_inventory("Token")))
```

<a id="command-not_equipped"></a>

### `cmd.not_equipped(item)`

Checks whether an item is not equipped.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1393`

```ts
cmd.not_equipped(item: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `item` | `string` | yes |  | Item name. |

<a id="command-not_in_bank"></a>

### `cmd.not_in_bank(item, quantity)`

Checks whether the bank does not contain an item.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1504`

```ts
cmd.not_in_bank(item: ScriptItemIdentifier, quantity: number | undefined): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `item` | [`ScriptItemIdentifier`](../item/#type-scriptitemidentifier) | yes |  | Item name or item id. |
| `quantity` | `number` \| `undefined` | yes |  | Quantity threshold. Defaults to `1`. |

<a id="command-not_in_cell"></a>

### `cmd.not_in_cell(cell)`

Checks whether the player is not in a cell.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1362`

```ts
cmd.not_in_cell(cell: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `cell` | `string` | yes |  | Cell name. |

<a id="command-not_in_combat"></a>

### `cmd.not_in_combat()`

Checks whether the player is not in combat.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1521`

```ts
cmd.not_in_combat(): ScriptCondition
```

<a id="command-not_in_house"></a>

### `cmd.not_in_house(item, quantity)`

Checks whether the house inventory does not contain an item.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1544`

```ts
cmd.not_in_house(item: string, quantity: number | undefined): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `item` | `string` | yes |  | Item name. |
| `quantity` | `number` \| `undefined` | yes |  | Quantity threshold. Defaults to `1`. |

<a id="command-not_in_inventory"></a>

### `cmd.not_in_inventory(item, quantity)`

Checks whether the inventory does not contain an item.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1446`

```ts
cmd.not_in_inventory(item: string, quantity: number | undefined): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `item` | `string` | yes |  | Item name. |
| `quantity` | `number` \| `undefined` | yes |  | Quantity threshold. Defaults to `1`. |

<a id="command-not_in_map"></a>

### `cmd.not_in_map(map)`

Checks whether the player is not in a map.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1828`

```ts
cmd.not_in_map(map: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `map` | `string` | yes |  | Map name. |

<a id="command-not_in_tempinventory"></a>

### `cmd.not_in_tempinventory(item, quantity)`

Checks whether the temp inventory does not contain an item.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1478`

```ts
cmd.not_in_tempinventory(item: string, quantity: number | undefined): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `item` | `string` | yes |  | Temporary item name. |
| `quantity` | `number` \| `undefined` | yes |  | Quantity threshold. Defaults to `1`. |

<a id="command-or"></a>

### `cmd.or(...conditions)`

Builds a logical OR condition expression.

`or` only creates an expression. It does not start a block by itself;
pass the result to `if`, `not`, `and`, or another `or`. Use `if_any`
when the entire conditional block is a flat OR of several expressions.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1338`

```ts
cmd.or(...conditions: ConditionInput[]): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `conditions` | [`ConditionInput`](../conditions/#type-conditioninput)[] | yes |  | Complete condition expressions to combine with logical OR. |

**Examples**

```js
cmd.if(cmd.or(cmd.in_inventory("Token"), cmd.in_bank("Token")))
```

<a id="command-player_aura_stacks"></a>

### `cmd.player_aura_stacks(player, aura, operator, count)`

Builds a player aura stack-count condition expression.

Stack count means repeated active aura instances with the same aura name.
Use [`cmd.player_aura_value`](#command-player_aura_value) to compare the optional numeric `val` in the aura payload.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1246`

```ts
cmd.player_aura_stacks(player: string, aura: string, operator: ScriptComparisonOperatorInput, count: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `player` | `string` | yes |  | Player name. |
| `aura` | `string` | yes |  | Aura name. |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `count` | `number` | yes |  | Aura stack count to compare against. |

**Examples**

```js
cmd.if(cmd.player_aura_stacks("Artix", "Some Aura", ">=", 3))
```

<a id="command-player_aura_value"></a>

### `cmd.player_aura_value(player, aura, operator, value)`

Builds a player aura value condition expression.

Aura value refers to the optional numeric `val` in the aura payload, which can be an
integer or float. Use [`cmd.player_aura_stacks`](#command-player_aura_stacks) to compare repeated aura
instances.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1218`

```ts
cmd.player_aura_value(player: string, aura: string, operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `player` | `string` | yes |  | Player name. |
| `aura` | `string` | yes |  | Aura name. |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | Aura value to compare against. Can be an integer or float. |

**Examples**

```js
cmd.if(cmd.player_aura_value("Artix", "Some Aura", ">=", 1))
```

<a id="command-player_count"></a>

### `cmd.player_count(operator, value)`

Builds a room player count condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1100`

```ts
cmd.player_count(operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | Player count to compare against. |

**Examples**

```js
cmd.if(cmd.player_count(">=", 3))
```

<a id="command-player_hp"></a>

### `cmd.player_hp(player, operator, value)`

Builds a named player HP condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1042`

```ts
cmd.player_hp(player: string, operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `player` | `string` | yes |  | Player name. |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | HP value to compare against. |

**Examples**

```js
cmd.if(cmd.player_hp("Artix", "<", 1000))
```

<a id="command-player_hp_percentage"></a>

### `cmd.player_hp_percentage(player, operator, value)`

Builds a named player HP percentage condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1061`

```ts
cmd.player_hp_percentage(player: string, operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `player` | `string` | yes |  | Player name. |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | HP percentage to compare against. |

**Examples**

```js
cmd.if(cmd.player_hp_percentage("Artix", "<=", 40))
```

<a id="command-player_in_cell"></a>

### `cmd.player_in_cell(player, cell)`

Checks whether a player is in a cell.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1589`

```ts
cmd.player_in_cell(player: string, cell: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `player` | `string` | yes |  | Player name. |
| `cell` | `string` | yes |  | Cell name. |

<a id="command-player_in_map"></a>

### `cmd.player_in_map(player)`

Checks whether a player is in the current map.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1573`

```ts
cmd.player_in_map(player: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `player` | `string` | yes |  | Player name. |

<a id="command-player_name_equals"></a>

### `cmd.player_name_equals(player)`

Checks whether the current player name matches a value.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1637`

```ts
cmd.player_name_equals(player: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `player` | `string` | yes |  | Expected player name. |

<a id="command-player_not_in_cell"></a>

### `cmd.player_not_in_cell(player, cell)`

Checks whether a player is not in a cell.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1621`

```ts
cmd.player_not_in_cell(player: string, cell: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `player` | `string` | yes |  | Player name. |
| `cell` | `string` | yes |  | Cell name. |

<a id="command-player_not_in_map"></a>

### `cmd.player_not_in_map(player)`

Checks whether a player is not in the current map.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1605`

```ts
cmd.player_not_in_map(player: string): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `player` | `string` | yes |  | Player name. |

<a id="command-quest_in_progress"></a>

### `cmd.quest_in_progress(questId)`

Checks whether a quest is in progress.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1684`

```ts
cmd.quest_in_progress(questId: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `questId` | `number` | yes |  | Quest id. |

<a id="command-quest_is_available"></a>

### `cmd.quest_is_available(questId)`

Checks whether a quest is available.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1716`

```ts
cmd.quest_is_available(questId: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `questId` | `number` | yes |  | Quest id. |

<a id="command-quest_not_available"></a>

### `cmd.quest_not_available(questId)`

Checks whether a quest is not available.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1732`

```ts
cmd.quest_not_available(questId: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `questId` | `number` | yes |  | Quest id. |

<a id="command-quest_not_in_progress"></a>

### `cmd.quest_not_in_progress(questId)`

Checks whether a quest is not in progress.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1700`

```ts
cmd.quest_not_in_progress(questId: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `questId` | `number` | yes |  | Quest id. |

<a id="command-target_hp"></a>

### `cmd.target_hp(operator, value)`

Builds a current target HP condition expression.

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:1269`

```ts
cmd.target_hp(operator: ScriptComparisonOperatorInput, value: number): ScriptCondition
```

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `operator` | `ScriptComparisonOperatorInput` | yes |  | Comparison operator. |
| `value` | `number` | yes |  | HP value to compare against. |

**Examples**

```js
cmd.if(cmd.target_hp("<", 5000))
```

## Referenced Types

<a id="type-conditioninput"></a>

### `ConditionInput`

**Source:** `app/src/renderer/windows/game/scripting/Commands/conditions.ts:45`

```ts
type ConditionInput =
  | ScriptCondition
  | boolean;
```

<a id="type-scriptaurametric"></a>

### `ScriptAuraMetric`

**Source:** `app/src/renderer/windows/game/scripting/Commands/commandDsl.ts:71`

```ts
type ScriptAuraMetric =
  | "value"
  | "stacks";
```

<a id="type-scriptcomparisonoperator"></a>

### `ScriptComparisonOperator`

**Source:** `app/src/renderer/windows/game/scripting/Commands/commandDsl.ts:52`

```ts
type ScriptComparisonOperator =
  | "eq" // ==
  | "ne" // !=
  | "lt" // <
  | "lte" // <=
  | "gt" // >
  | "gte";
```

<a id="type-scriptcondition"></a>

### `ScriptCondition`

Serializable condition expression produced by the scripting DSL.

Condition builder methods in `conditions.ts` create these plain objects
during script compilation. Control-flow commands such as [`cmd.if(...)`](#command-if)
record the object in a `ScriptInstruction`, and `evaluateScriptCondition`
interprets it at runtime by dispatching on `_tag`.

Example flow:
`cmd.if(cmd.hp("<", 1000))`
  1. [`cmd.hp`](#command-hp) builds `{ _tag: "PlayerMetric", metric: "hp", ... }`.
  2. [`cmd.if`](#command-if) stores that object as the first instruction argument.
  3. `ifCommand` calls `evaluateScriptCondition` with that argument.
  4. The `"PlayerMetric"` case reads live HP and compares it to `1000`.

**Source:** `app/src/renderer/windows/game/scripting/Commands/commandDsl.ts:89`

```ts
type ScriptCondition =
  | {
      readonly _tag: "All";
      readonly conditions: readonly unknown[];
    }
  | {
      readonly _tag: "Any";
      readonly conditions: readonly unknown[];
    }
  | {
      readonly _tag: "PlayerMetric";
      readonly metric: ScriptPlayerMetric;
      readonly operator: ScriptComparisonOperator;
      readonly value: number;
    }
  | {
      readonly _tag: "SelfNumberMetric";
      readonly metric: "gold" | "level";
      readonly operator: ScriptComparisonOperator;
      readonly value: number;
    }
  | {
      readonly _tag: "PlayerNamedMetric";
      readonly metric: "hp" | "hp_percent";
      readonly player?: string;
      readonly operator: ScriptComparisonOperator;
      readonly value: number;
    }
  | {
      readonly _tag: "AnyPlayerMetric";
      readonly metric: "hp_percent";
      readonly operator: ScriptComparisonOperator;
      readonly value: number;
    }
  | {
      readonly _tag: "PlayerAuraPresence";
      readonly player?: string;
      readonly aura: string;
    }
  | {
      readonly _tag: "PlayerAuraMetric";
      readonly metric: ScriptAuraMetric;
      readonly player?: string;
      readonly aura: string;
      readonly operator: ScriptComparisonOperator;
      readonly value: number;
    }
  | {
      readonly _tag: "PlayerCount";
      readonly cell?: string;
      readonly operator: ScriptComparisonOperator;
      readonly value: number;
    }
  | {
      readonly _tag: "MonsterMetric";
      readonly metric: ScriptMonsterMetric;
      readonly target: string;
      readonly operator: ScriptComparisonOperator;
      readonly value: number;
    }
  | {
      readonly _tag: "MonsterPresence";
      readonly monster: string;
      readonly expected: boolean;
    }
  | {
      readonly _tag: "InventoryContains";
      readonly location: ScriptInventoryLocation;
      readonly item: ItemIdentifierToken;
      readonly quantity: number;
      readonly expected: boolean;
    }
  | {
      readonly _tag: "ItemState";
      readonly item: string;
      readonly quantity?: number;
      readonly state: "equipped" | "maxed" | "dropped" | "can_buy";
      readonly expected: boolean;
    }
  | {
      readonly _tag: "BooleanState";
      readonly state: "has_target" | "in_combat" | "member";
      readonly expected: boolean;
    }
  | {
      readonly _tag: "Cell";
      readonly cell: string;
      readonly expected: boolean;
    }
  | {
      readonly _tag: "Map";
      readonly map: string;
      readonly expected: boolean;
    }
  | {
      readonly _tag: "PlayerLocation";
      readonly player: string;
      readonly cell?: string;
      readonly expected: boolean;
    }
  | {
      readonly _tag: "PlayerName";
      readonly player: string;
      readonly expected: boolean;
    }
  | {
      readonly _tag: "FactionRank";
      readonly faction: string;
      readonly operator: ScriptComparisonOperator;
      readonly value: number;
    }
  | {
      readonly _tag: "ClassRank";
      readonly className: string;
      readonly operator: ScriptComparisonOperator;
      readonly value: number;
    }
  | {
      readonly _tag: "QuestState";
      readonly questId: number;
      readonly state: "available" | "can_complete" | "in_progress";
      readonly expected: boolean;
    }
  | {
      readonly _tag: "TargetHp";
      readonly operator: ScriptComparisonOperator;
      readonly value: number;
      readonly upper?: number;
    }
  | {
      readonly _tag: "Custom";
      readonly name: string;
      readonly args: ReadonlyArray<unknown>;
    }
  | {
      readonly _tag: "Not";
      readonly condition: unknown;
    };
```

<a id="type-scriptinventorylocation"></a>

### `ScriptInventoryLocation`

**Source:** `app/src/renderer/windows/game/scripting/Commands/commandDsl.ts:72`

```ts
type ScriptInventoryLocation =
  | "bank"
  | "house"
  | "inventory"
  | "temp";
```

<a id="type-scriptmonstermetric"></a>

### `ScriptMonsterMetric`

**Source:** `app/src/renderer/windows/game/scripting/Commands/commandDsl.ts:70`

```ts
type ScriptMonsterMetric =
  | "monster_health"
  | "monster_health_percent";
```

<a id="type-scriptplayermetric"></a>

### `ScriptPlayerMetric`

**Source:** `app/src/renderer/windows/game/scripting/Commands/commandDsl.ts:69`

```ts
type ScriptPlayerMetric =
  | "hp"
  | "hp_percent"
  | "mp"
  | "mp_percent";
```
