---
title: ShopItemData
---

| Name | Type | Optional | Description |
|------|------|----------|-------------|
| bBank | `number` |  |  |
| bCoins | `number` |  |  |
| bEquip | `number` |  |  |
| bStaff | `number` |  |  |
| bTemp | `number` |  |  |
| bUpg | `number` |  |  |
| bWear | `number` |  |  |
| CharID | `number` |  |  |
| CharItemID | `number` |  |  |
| dPurchase | `string` |  |  |
| EnhDPS | `number` |  |  |
| EnhID | `number` |  |  |
| EnhLvl | `number` |  |  |
| EnhPatternID | `number` |  |  |
| EnhRng | `number` |  |  |
| EnhRty | `number` |  |  |
| iCost | `number` |  |  |
| iDPS | `number` |  |  |
| iHrs | `number` |  |  |
| iLvl | `number` |  |  |
| iQty | `number` |  |  |
| iRng | `number` |  |  |
| iRty | `number` |  |  |
| iStk | `number` |  |  |
| ItemID | `number` |  |  |
| iType | `number` |  |  |
| PatternID | `number` \| `string?` | ✓ |  |
| ProcID | `number`? | ✓ |  |
| sDesc | `string` |  |  |
| sElmt | `string` |  |  |
| sES | `string` |  |  |
| sFile | `string` |  |  |
| sIcon | `string` |  |  |
| sLink | `string` |  |  |
| sMeta | `string` |  |  |
| sName | `string` |  |  |
| sType | `string` |  |  |
| bEnhShop | `boolean`? | ✓ | Enhancement-specific properties |
| bHouse | `string` |  | Whether the item can be placed in a house. |
| FactionID | `string` |  | Faction ID associated with the item. |
| iClass | `string` |  |  |
| iQSindex | `string` |  |  |
| iQSvalue | `string` |  |  |
| iQtyRemain | `string` |  |  |
| iReqCP | `string` |  |  |
| iReqRep | `string` |  |  |
| ItemProcID | `string`? | ✓ | Proc ID for Forge/Awe weapon enhancements |
| sFaction | `string` |  | Faction associated with the item. |
| ShopItemID | `string` |  | Shop item id. |
| sQuest | `string`? | ✓ |  |
| turnin | `{ iQty: string; ItemID: string; sName: string }`[]? | ✓ | Items required to merge this item. |