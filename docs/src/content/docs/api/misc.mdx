...
### cmd.do_simple_looptaunt
Performs loop taunt pattern on the specified target(s).

:::note[Remarks]
Player 1 is t1, Player 2 is t2, etc. The taunt order will be t1 -> t2 -> t3... -> tN -> t1 -> t2...
...
| Parameter | Type | Description |
|-----------|------|-------------|
| target | `string` | The name or monMapId of the target(s). If multiple, separate by comma. |
| playerIndex | `number` | The index at which the player will taunt, 1-based. |
| maxPlayers | `number` | The total number of players that will be taunting. |

<Code code={`cmd.is_player_number(1)
cmd.do_simple_looptaunt("Lava Golem", 1, 2)
cmd.is_player_number(2)
cmd.do_simple_looptaunt("Darkon the Conductor", 2, 2)
cmd.kill("Darkon the Conductor")`} lang="js" />
...
| Parameter | Type | Description |
|-----------|------|-------------|
| username | `null` \| `string` | The username of the account. If set to null, the current username (as shown to the bot) will be used. |
| password | `null` \| `string` | The password of the account. If set to null, the current password (as shown to the bot) will be used. |
| server | `null` \| `string` | The server to log into. If set to null, the current server will be used. |

<Code code={`cmd.use_autorelogin(null, null, 'Twig') // uses the current username and password, but sets the server to Twig
cmd.use_autorelogin('myusername', 'mypassword', null) // uses the current server, but sets the username and password`} lang="js" />
