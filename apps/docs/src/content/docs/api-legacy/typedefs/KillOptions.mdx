...
| signal | `AbortSignal`? | ✓ | Optional AbortSignal that can be used to abort the kill operation early. When the signal is aborted, the kill method will immediately stop and resolve. |
