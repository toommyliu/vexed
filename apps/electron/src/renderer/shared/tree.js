		...
		siblings(element = this.parent.querySelector('a, details')) {
			if (this.parent.contains(element)) {
				...
				return [...element.parentElement.children]
					.filter((e) => {
						return (
							e.dataset.type === SimpleTree.FILE ||
							e.dataset.type === SimpleTree.FOLDER
						);
					})
					.map((e) => {
	...
	class AsyncTree extends SimpleTree {
		...
		folder(...args) {
			...
			details.addEventListener('toggle', (e) => {
				this.emit(
					details.dataset.loaded === 'false' && details.open
						? 'fetch'
						: 'open',
					summary,
				);
			...
			for (const e of es) {
				if (validate(e)) {
					...
					if (parent.open) {
						...
						window.setTimeout(() => {
							this.once('open', () =>
								this.browse(validate, this.children(parent)),
							);
							this.open(parent);
						}, 0);
						return;
	...
	class SelectTree extends AsyncTree {
		...
		focus(target) {
			...
			this.id = window.setTimeout(
				() => document.hasFocus() && target.focus(),
				100,
			);
		}
		select(target) {
			...
			[...this.parent.querySelectorAll('.selected')].forEach((e) =>
		...
		json(array, parent) {
			array.forEach((item) => {
				if (item.type === SimpleTree.FOLDER) {
					const folder = this[
						item.asynced ? 'asyncFolder' : 'folder'
					](item, parent);
					if (item.children) {
