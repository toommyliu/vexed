<script lang="ts" module>
  ...
  export const badgeVariants = tv({
    base: [
      "relative inline-flex shrink-0 items-center justify-center gap-1 whitespace-nowrap rounded-sm border border-transparent font-medium outline-none transition-shadow",
    ...
    variants: {
      size: {
        default:
          "h-5.5 min-w-5.5 px-[calc(theme(spacing[1])-1px)] text-sm sm:h-4.5 sm:min-w-4.5 sm:text-xs",
        lg:
          "h-6.5 min-w-6.5 px-[calc(theme(spacing[1.5])-1px)] text-base sm:h-5.5 sm:min-w-5.5 sm:text-sm",
        sm: "h-5 min-w-5 rounded-[.25rem] px-[calc(theme(spacing[1])-1px)] text-xs sm:h-4 sm:min-w-4 sm:text-[.625rem]",
      },
      variant: {
        default:
