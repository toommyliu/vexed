<script lang="ts">
  import { cn } from "$lib/utils";
  import { tv, type VariantProps } from "tailwind-variants";
  import type { HTMLAttributes } from "svelte/elements";
  import { getInputGroupContext } from "./context.js";

  const inputGroupAddonVariants = tv({
    base: "flex h-auto cursor-text select-none items-center justify-center gap-2 leading-none [&>kbd]:rounded-[calc(var(--radius)-5px)] [&>svg:not([class*='size-'])]:size-4 [&_svg]:opacity-80",
    variants: {
      align: {
        "block-end":
          "order-last w-full justify-start px-[calc(theme(spacing.3)-1px)] pb-[calc(theme(spacing.3)-1px)] [.border-t]:pt-[calc(theme(spacing.3)-1px)] [[data-size=sm]+&]:px-[calc(theme(spacing[2.5])-1px)]",
        "block-start":
          "order-first w-full justify-start px-[calc(theme(spacing.3)-1px)] pt-[calc(theme(spacing.3)-1px)] [.border-b]:pb-[calc(theme(spacing.3)-1px)] [[data-size=sm]+&]:px-[calc(theme(spacing[2.5])-1px)]",
        "inline-end":
          "order-last pe-[calc(theme(spacing.3)-1px)] [[data-size=sm]+&]:pe-[calc(theme(spacing[2.5])-1px)]",
        "inline-start":
          "order-first ps-[calc(theme(spacing.3)-1px)] [[data-size=sm]+&]:ps-[calc(theme(spacing[2.5])-1px)]",
      },
    },
    defaultVariants: {
      align: "inline-start",
    },
  });

  type Align = VariantProps<typeof inputGroupAddonVariants>["align"];

  interface Props extends HTMLAttributes<HTMLDivElement> {
    align?: Align;
    ref?: HTMLDivElement;
  }

  let {
    class: className,
    align = "inline-start",
    ref = $bindable(),
    children,
    ...props
  }: Props = $props();

  const ctx = getInputGroupContext();

  // Signal the parent InputGroup when this addon uses block alignment
  $effect(() => {
    if (ctx && (align === "block-start" || align === "block-end")) {
      ctx.hasBlockAddon.value = true;
      return () => {
        ctx.hasBlockAddon.value = false;
      };
    }
  });

  // Detect a direct-child button so we can pull in the addon edge to avoid double-padding.
  // Uses :scope > button (Chrome 87 safe) instead of CSS has-[>button].
  let hasButton = $state(false);
  let hasBadge = $state(false);

  $effect(() => {
    if (!ref) return;
    const el = ref;
    const update = () => {
      hasButton = !!el.querySelector(":scope > button, :scope > [role='button']");
      hasBadge = !!el.querySelector(":scope > [data-slot='badge']");
    };
    update();
    const observer = new MutationObserver(update);
    observer.observe(el, { childList: true });
    return () => observer.disconnect();
  });

  function handleMouseDown(e: MouseEvent) {
    const target = e.target as HTMLElement;
    const isInteractive = target.closest("button, a, input, select, textarea, [role='button'], [role='combobox'], [role='listbox']");
    if (isInteractive) return;
    e.preventDefault();
    const currentTarget = e.currentTarget as HTMLElement;
    const parent = currentTarget.parentElement;
    const input = parent?.querySelector<HTMLInputElement | HTMLTextAreaElement>("input, textarea");
    if (input && !parent?.querySelector("input:focus, textarea:focus")) {
      input.focus();
    }
  }
</script>

<!-- svelte-ignore a11y_no_static_element_interactions -->
<div
  bind:this={ref}
  class={cn(
    inputGroupAddonVariants({ align }),
    align === "inline-end" && hasButton && "-me-2",
    align === "inline-end" && hasBadge && !hasButton && "-me-1.5",
    align === "inline-start" && hasButton && "-ms-2",
    align === "inline-start" && hasBadge && !hasButton && "-ms-1.5",
    className,
  )}
  data-align={align}
  data-slot="input-group-addon"
  onmousedown={handleMouseDown}
  {...props}
>
  {@render children?.()}
</div>
